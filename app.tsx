/**
 * @file JPMorgan Watchlist Application
 * Client-side static feed viewer for api/jpmorgan/** with multi-ETF Watchlist
...
const INDEX_URL = './api/jpmorgan/index.json';
const THEME_KEY = 'jpmorgan-theme';
const SELECTED_KEY = 'jpmorgan-selected-etfs';
const BLACKLIST_KEY = 'jpmorgan-blacklisted-etfs';
const ACTIVE_FUND_KEY = 'jpmorgan-active-fund';
const FILTERS_KEY = 'jpmorgan-tab-filters';
const LEGACY_FILTERS_KEY = 'jpmorgan-searches'; // pre-rename key, migrated at boot
const SORTS_KEY = 'jpmorgan-tab-sorts';
const SITE_STATE_KEY = 'jpmorgan-site-state';
...
const COLUMN_TOOLTIPS: Record<string, string> = {
  ...
  'Fund Name': 'Fund Name — Official legal name of the JPMorgan exchange-traded fund (ETF), as published in the am.jpmorgan.com ETF fund explorer.',
  Category: 'Category — am.jpmorgan.com groups its ETFs by asset class ("U.S. Equity", "Fixed Income Taxable", "International Equity"); the tab shows the asset class, the asset class plus benchmark is kept in meta.json.',
  Name: 'Security Name — Full registered legal name of the company or underlying financial asset.',
  Identifier: 'CUSIP / SEDOL — Security identifier as published in the am.jpmorgan.com daily holdings (CUSIP for US securities, SEDOL for foreign ones; or taken from the N-PORT filing when that fallback supplied the sheet). Positions without an exchange ticker (bonds, cash, futures) are identified in the Watchlist by this.',
  ...
  Type: 'Category — the am.jpmorgan.com asset class (see the Category column). Same source as the category tabs.',
  Expense: 'Gross Expense Ratio — Total annual fund operating expenses as a % of assets.',
  'Dividend Yield': 'Dividend Yield — the 12-month rolling dividend yield published by am.jpmorgan.com (product-data) when present; otherwise indicated (latest distribution per share x payments per year / market price) from the published dividend schedule.',
  'SEC Yield': 'SEC Yield (30-Day) — The 30-day SEC yield as published by am.jpmorgan.com (daily product-data figure, month-end fund-explorer figure otherwise; the 7-day SEC yield for the money market ETF); "—" when JPMorgan publishes none for the fund.',
  'YTD Return': 'YTD Return — Official JPMorgan NAV total return since the start of the year (month-end series); for funds without a published figure, derived from the official daily NAV history with distributions reinvested.',
  'TR 1Y': 'TR 1Y (1-Year Total Return) — Official JPMorgan NAV total return over the past year, including reinvested distributions (month-end series); derived from the daily NAV history only when JPMorgan publishes none.',
  'TR 3Y': 'TR 3Y (3-Year Total Return) — Cumulative NAV total return over 3 years as published by am.jpmorgan.com (cumulative returns block); otherwise derived exactly from the 3Y CAGR: (1 + CAGR 3Y)^3 - 1.',
  'TR 5Y': 'TR 5Y (5-Year Total Return) — Cumulative NAV total return over 5 years as published by am.jpmorgan.com (cumulative returns block); otherwise derived exactly from the 5Y CAGR: (1 + CAGR 5Y)^5 - 1.',
  'TR 10Y': 'TR 10Y (10-Year Total Return) — Cumulative NAV total return over 10 years as published by am.jpmorgan.com (cumulative returns block); otherwise derived exactly from the 10Y CAGR: (1 + CAGR 10Y)^10 - 1.',
  'CAGR 3Y': 'CAGR 3Y (3-Year Compound Annual Growth Rate) — Official JPMorgan annualized NAV total return over 3 years (month-end series).',
  'CAGR 5Y': 'CAGR 5Y (5-Year Compound Annual Growth Rate) — Official JPMorgan annualized NAV total return over 5 years (month-end series).',
  'CAGR 10Y': 'CAGR 10Y (10-Year Compound Annual Growth Rate) — Official JPMorgan annualized NAV total return over 10 years (month-end series).',
  YTD: 'YTD NAV total return, official JPMorgan month-end series (derived from the daily NAV history when unpublished).',
  '1Y': '1-year official JPMorgan NAV total return, month-end series.',
  '3Y': '3-year average annual NAV total return (CAGR), official JPMorgan month-end series.',
  '5Y': '5-year average annual NAV total return (CAGR), official JPMorgan month-end series.',
  '10Y': '10-year average annual NAV total return (CAGR), official JPMorgan month-end series.',
  'SI Ann.': 'Since-inception annualized NAV total return, official JPMorgan month-end series (blank for funds younger than one year: JPMorgan publishes their since-inception return cumulatively, not annualized).',
  ...
  Frequency: 'Frequency — sortable payment cadence from the am.jpmorgan.com dividend-frequency code (MDEC = monthly, QDEC = quarterly, YDEC = annual), inferred from the published dividend schedule when no code exists: 01 - Monthly, 04 - Quarterly, 06 - Semi-annually, 12 - Annually; 00 denotes unavailable/unknown and 99 denotes irregular.',
  ...
  'Market Price': 'Closing market price on that date, as published by am.jpmorgan.com.',
  'Premium/Discount': 'Premium / Discount — closing market price versus NAV on that date (%), as published by am.jpmorgan.com.',
...
function exportFileName(scope: string, extension: string): string {
  ...
  return `jpmorgan-${scope.toLowerCase().replace(/\s+/g, '-')}-${stamp}.${extension}`;
}
...
// =========================================================================
// 4. Static API loading & paginated sheets (api/jpmorgan/**, iShares-style)
// =========================================================================
...
async function loadCatalog(): Promise<void> {
  setStatus('Loading JPMorgan ETF data from api/jpmorgan/index.json…', 'info');
...
async function loadFundMeta(ticker: string): Promise<any> {
  ...
  const request = (async () => {
    try {
      const meta = await fetchJson(`./api/jpmorgan/funds/${encodeURIComponent(ticker)}/meta.json`);
...
async function fetchPage(ticker: string, pagePath: string): Promise<{ headers: string[]; rows: string[][] }> {
  ...
  const page = await fetchJson(`./api/jpmorgan/funds/${encodeURIComponent(ticker)}/${path}`);
  ...
  if (!state.selected.size) {
    ...
    el.tableBody.innerHTML = `<tr><td colspan="8" class="py-12 text-center text-slate-400 dark:text-slate-500">${
      loading
        ...
        : 'Holdings data is not available yet. Run bun ./scripts/update-data.ts to refresh the JPMorgan feed.'
...
function renderOverviewTable(fund: FundRow): void {
  ...
  const overview: Array<{ section: string; metric: string; value: unknown }> = [
    ...
    { section: 'Distributions', metric: 'SEC Yield (30-day)', value: meta && meta.yields ? (meta.yields.secYieldText || '—') : (fund.secYield === null || fund.secYield === undefined ? 'not published by am.jpmorgan.com for this fund' : `${fund.secYield.toFixed(2)}%`) },
...
function renderDistributionsTable(fund: FundRow): void {
  ...
  el.tickerCount.textContent = fund.ticker;
  renderSubtitle(`${fund.ticker} distributions · dividend schedule published by am.jpmorgan.com (ex-date, amount; the last twelve payments); frequency from the published dividend-frequency code, inferred from the cadence otherwise.`);
}
...
function renderSubtitle(text?: string): void {
  ...
  const countsText = state.counts
    ...
    : '';
  const base = text ? String(text) : 'Search JPMorgan ETFs, select ETFs via the “Use” checkbox, then use the Watchlist tab.';
  ...
  el.subtitle.innerHTML = `
    ...
    <span class="block sm:inline">·${generated ? ` updated ${escapeHtml(generated)}` : ''}${countsText ? ` · ${escapeHtml(countsText)}.` : '.'} Data: <a href="./api/jpmorgan/index.json" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">api/jpmorgan/index.json</a> generated from <a href="https://am.jpmorgan.com/us/en/asset-management/adv/products/fund-explorer/etf" target="_blank" rel="noopener noreferrer" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">am.jpmorgan.com ETF fund data</a> + SEC EDGAR N-PORT-P + Yahoo Finance (fallbacks)</span>
  `;
...
function init(): void {
  ...
  void loadCatalog().catch(error => {
    ...
    setStatusRow(`Unable to load api/jpmorgan/index.json: ${message}. Run bun ./scripts/update-data.ts and serve the folder (for example bunx serve . -p 1234).`, 'error');
  });
}
