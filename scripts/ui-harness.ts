/**
 * Headless harness for the JPMorgan UI (index.html + app.tsx).
 *
 ...
 * generated feed from api/jpmorgan/. That lets bun test exercise the exact
...
const API_ROOT = path.join(REPO_ROOT, "api", "jpmorgan");
...
function createFeedFetch(stats: FetchStats) {
  return async function fetchStub(input: string): Promise<any> {
    ...
    if (!relative.startsWith("api/jpmorgan/")) return notFound();
 ...
 * feed) also count as missing (contract §5, JPMorgan extension).
 */
