/**
 * Acceptance tests for the JPMorgan ETF UI contract (index.html + app.tsx).
 *
 ...
 * that serves the actual generated feed from api/jpmorgan/. Expected
...
test("6. rapid overlapping selections load without duplicate or skipped pages", async () => {
  ...
  for (const ticker of ["AGGY", "AGZD"]) {
    ...
    expect(app.stats.counts.get(`./api/jpmorgan/funds/${ticker}/meta.json`) ?? 0).toBe(1);
  }
