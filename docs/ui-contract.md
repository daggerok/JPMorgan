...
- **All ETFs** (`All`) plus one tab per JPMorgan asset class (category).
...
| Key | Content |
| --- | --- |
| `jpmorgan-selected-etfs` | JSON array of selected ETF tickers |
| `jpmorgan-active-fund` | active fund ticker (removed when none) |
| `jpmorgan-blacklisted-etfs` | JSON array of blacklisted tickers |
| `jpmorgan-tab-sorts` | JSON map `tab -> { key, dir }` of **explicitly chosen** sorts |
| `jpmorgan-tab-filters` | JSON map `tab -> query` of **explicitly entered** filters |
| `jpmorgan-site-state` | auxiliary state: `sheetFilter` mirror of the per-tab filter map plus the last `activeTab` |
| `jpmorgan-theme` | `dark` / `light` |
| `jpmorgan-searches` | **legacy** pre-rename filter key; migrated at boot and removed on first write |
...
- Every tab (All ETFs, category tabs, Watchlist, Overview, Holdings, History,
  ...
  direction in `jpmorgan-tab-sorts`.
...
- Every tab remembers its own search query in `jpmorgan-tab-filters`.
...
- `jpmorgan-site-state.sheetFilter` mirrors the dedicated filter map so
...
- `jpmorgan-selected-etfs` / `jpmorgan-active-fund` in localStorage.
...
- The Watchlist table mirrors this: `Loading holdings of N selected ETFs…`
  ...
  not available yet. Run bun ./scripts/update-data.ts to refresh the JPMorgan feed.`
...
The JPMorgan feed publishes one `Identifier` column per holding row
...
**JPMorgan extension:** some EDGAR N-PORT filings publish the literal all-
...
- `api/jpmorgan/index.json` — catalog; `funds[].holdings`/`funds[].history`
  agree with the per-fund manifests (validated by the acceptance tests' feed
  oracle, test 15).
- `api/jpmorgan/funds/<TICKER>/meta.json` — `holdings.pages[]` and
...
- Page rows are keyed by the literal `headers` strings published in each page
  ...
  funds; history: `Date`, `NAV`, `Market Price`, `Premium/Discount` from the
  official am.jpmorgan.com history, or `Date`, `Close`, `Adj Close`, `Volume`
  when the Yahoo chart fallback supplied the sheet).
- Distributions use the actual ex-date from the am.jpmorgan.com dividend
  schedule (Yahoo dividend events in the fallback path), rendered under the
  `Ex-Date` column.
...
file-backed `fetch` over `api/jpmorgan/`) and covers:
