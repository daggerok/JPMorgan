...
The updater's prose values (`Monthly`, `Quarterly`, `Semi-annually`,
`Annually` — decoded from JPMorgan's dividend-frequency codes such as `MDEC`,
inferred from the published dividend schedule otherwise) are mapped through
